...
export function registerTools(server: any) {
  ...
  // organize_action - Get suggestions for organizing and restructuring actions
  server.tool(
    "organize_action",
    "Analyze an action and provide suggestions for better organization in the hierarchy",
    {
    ...
    async ({ 
      ...
      try {
        console.log(`[MCP organize_action] Analyzing organization for action: ${action_id}`);
        ...
        // Use ActionsService to get organization suggestions
        ...
        let message = `🗂️ **Organization Suggestions for:** "${result.action.title}"\n`;
        message += `🔍 **Analysis Scope:** ${scope.replace('_', ' ')}\n`;
        message += `⚡ **Performance:** ${performance}ms total (${result.metadata?.processingTimeMs || 0}ms processing)\n`;
        message += `📊 **Found:** ${result.suggestions.length} suggestions (analyzed ${result.metadata?.analyzedCount || 0} items)\n\n`;

        if (result.suggestions.length === 0) {
          message += `✨ **No suggestions found** - The action appears to be well-organized!\n\n`;
        } else {
          message += `✅ **Suggestions:**\n\n`;

          result.suggestions.forEach((suggestion, index) => {
            const confidenceBar = '█'.repeat(Math.floor(suggestion.confidence * 10)) + '░'.repeat(10 - Math.floor(suggestion.confidence * 10));
            ...
            switch (suggestion.type) {
              case 'move':
                message += `   📍 Target: ${suggestion.target_parent_title} (ID: ${suggestion.target_parent_id})\n`;
                break;
              case 'rename':
                message += `   ✏️ New title: "${suggestion.new_title}"\n`;
                break;
              case 'split':
                if (suggestion.suggested_actions) {
                  message += `   🔪 Split into:\n`;
                  suggestion.suggested_actions.forEach((action, idx) => {
                    message += `      ${idx + 1}. ${action.title}\n`;
                  });
                }
                break;
              case 'merge':
                message += `   🔗 Merge with: ${suggestion.merge_target_title} (ID: ${suggestion.merge_target_id})\n`;
                break;
              case 'reorder':
                message += `   ↕️ New position: ${suggestion.new_position}\n`;
                break;
            }
          ...
          // Add usage instructions
          message += `📝 **Usage Instructions:**\n`;
          message += `• Review each suggestion and its confidence score\n`;
          message += `• Use join_family to move actions to different parents\n`;
          message += `• Use update_action to rename actions\n`;
          message += `• Use create_action to split actions into multiple parts\n`;
          message += `• Consider the reasoning provided for each suggestion\n`;
        }

        message += `\n🧠 **Powered by:** AI-driven organizational analysis`;
        ...
        console.error('[MCP organize_action] Error:', error);
        return {
          content: [
            {
              type: "text",
              text: `Error analyzing organization: ${error instanceof Error ? error.message : "Unknown error"}`,
            },
          ],
        };
      }
...
export const toolCapabilities = {
  ...
  organize_action: {
    description: "Analyze an action and provide suggestions for better organization in the hierarchy",
  },
};