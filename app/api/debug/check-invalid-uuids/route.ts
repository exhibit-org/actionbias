...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check for actions with invalid UUIDs in id field
    ...
    const invalidParentIds = await sql`
      SELECT e.src, e.dst, a.title
      FROM edges e
      JOIN actions a ON a.id = e.src
      WHERE e.kind = 'parent'
      AND (
        e.dst !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
        OR e.src !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
      )
      LIMIT 10;
    `;
