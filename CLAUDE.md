...
This is a Next.js MCP (Model Context Protocol) server that implements done.engineering - an AI-forward planning platform for persistent task management across LLM conversations. The server uses the `@vercel/mcp-adapter` to expose intelligent action management tools via HTTP endpoints, including AI-powered semantic action placement, hierarchical task organization, and dependency tracking.

## Architecture

- **Core MCP Handler**: `app/mcp/[transport]/route.ts` contains the main MCP server setup using the Vercel adapter
...
- **Tool Definition**: MCP tools are defined using Zod schemas in `lib/mcp/tools.ts`
...
- Tools are defined in the `createMcpHandler` callback in `app/mcp/[transport]/route.ts`
- MCP tools are registered via `registerTools()` from `lib/mcp/tools.ts`
...
### Transport Endpoints
- SSE: `/mcp/sse` endpoint for Server-Sent Events transport
- HTTP Streaming: `/mcp/mcp` endpoint for streamable HTTP transport
...
The `lib/services/` directory contains the business logic layer:
...
- **SummaryService** (`summary.ts`): AI-generated action summaries
- **CompletionContextService** (`completion-context.ts`): Rich completion stories with editorial content
- **ClassificationService** (`classification.ts`): AI-powered action categorization and placement
...
The test suite uses Jest with custom setup for different environments:

- **Setup Files**:
  - `jest.setup.js`: Basic Jest configuration
  - `jest.setup.web.cjs`: Web-specific setup for React components
  - Database setup is conditionally loaded only for tests that need it

- **Test Categories**:
  - Component tests: React Testing Library for UI components
  - Service tests: Unit tests with mocked dependencies
  - Integration tests: End-to-end API and database tests
  - MCP tests: Protocol compliance and tool functionality

- **Key Testing Notes**:
  - Jest is configured with limited workers (`maxWorkers: 2`) to prevent crashes
...
- Implement HTTP methods (GET, POST, PUT, DELETE) as named exports
- Use ActionsService for database operations
- Include proper error handling and response formatting

### Adding New MCP Tools
1. Define the tool schema in `lib/mcp/tools.ts` using Zod
2. Implement the handler function using the appropriate service
3. Add the tool to the capabilities list
4. Register the tool in the `registerTools()` function
5. Add tests in `__tests__/mcp/` directory

### Database Migrations
- Migrations are handled by Drizzle Kit
- Use `pnpm db:generate` to create new migrations
- Production deployments run migrations automatically via `pnpm db:migrate:vercel`
- PGlite is used for development and testing (no external dependencies)
